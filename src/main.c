...
key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
  ...
  switch (key) {
    case GLFW_KEY_W:
      ws->key_input = action == GLFW_PRESS ? (ws->key_input | INPUT_W)
                                           : (ws->key_input & ~INPUT_W);
      break;
    case GLFW_KEY_S:
      ws->key_input = action == GLFW_PRESS ? (ws->key_input | INPUT_S)
                                           : (ws->key_input & ~INPUT_S);
      break;
    case GLFW_KEY_A:
      ws->key_input = action == GLFW_PRESS ? (ws->key_input | INPUT_A)
                                           : (ws->key_input & ~INPUT_A);
      break;
    case GLFW_KEY_D:
      ws->key_input = action == GLFW_PRESS ? (ws->key_input | INPUT_D)
                                           : (ws->key_input & ~INPUT_D);
      break;
    case GLFW_KEY_Q:
      ws->key_input = action == GLFW_PRESS ? (ws->key_input | INPUT_Q)
                                           : (ws->key_input & ~INPUT_Q);
      break;
    case GLFW_KEY_E:
...
mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
  ...
  switch (button) {
    case GLFW_MOUSE_BUTTON_LEFT:
      ws->mouse_input = action == GLFW_PRESS ? (ws->mouse_input | INPUT_MB0)
                                             : (ws->mouse_input & ~INPUT_MB0);
      break;
    case GLFW_MOUSE_BUTTON_RIGHT:
      ws->mouse_input = action == GLFW_PRESS ? (ws->mouse_input | INPUT_MB1)
                                             : (ws->mouse_input & ~INPUT_MB1);
      break;
  }
}
...
void
update_view(camera *cam, window_state *ws) {
  float speed = 0.15f;
  vec3f offset_pos = {};

  if (ws->key_input & INPUT_W) {
    offset_pos.z = -speed;
  }
  if (ws->key_input & INPUT_S) {
    offset_pos.z = speed;
  }
  if (ws->key_input & INPUT_D) {
    offset_pos.x = -speed;
  }
  if (ws->key_input & INPUT_A) {
    offset_pos.x = speed;
  }
  if (ws->key_input & INPUT_Q) {
    offset_pos.y = -speed;
  }
  if (ws->key_input & INPUT_E) {
    offset_pos.y = speed;
  }
...
main(int ac, const char **av) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (frame_no % 10 == 0) {
      snprintf(window_title, 128, "splat.c | %.1f (%.3f / %.3f)", fps,
               trans_time, render_time);
